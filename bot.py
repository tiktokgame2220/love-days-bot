...
async def list_holidays(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for holiday, days_until, holiday_date in holidays_with_days:
        if days_until == 0:
...
async def bot_birthday_info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    bot_birthday = datetime.strptime(f"15.05.{current_date.year}", "%d.%m.%Y").date()
    ...
    if days_until == 0:
        ...
        message = f"🤖 День создания бота: 15 мая\n📅 Осталось ждать: {days_until} дней"
